...
### Introspection
```bash
php cli route:list [--method=GET] [--path=/api] [--json]
```
...
│
├─ Introspection Commands (1)
│  └─ route:list — dump the compiled route manifest (filters + --json)
│
...
- ✅ `route:list` reads `var/cache/manifests/route-manifest.php` (no module.json re-parse)
- ✅ Colour-coded methods; project routes flagged with the plugin route they override
- ✅ `--method` / `--path` filters; deterministic path-then-method sort
- ✅ `--json` for machine parsing; clear "boot the app once" hint when the manifest is missing
